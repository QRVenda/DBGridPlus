{*******************************************************************************
* CRÉDITOS       : TODOS OS CRÉDITOS ao autor do "TSMDBGrid"                   *
    ...
    begin
      if (Columns[BCol] is TPlusDBColumn) then
        ...
        begin
          ...
          if (Field is TDateTimeField) and (Columns[BCol].DisplayFormat <> '') and  //and (TDateTimeField(Field).DisplayFormat <> '') and
    ...
    begin
      ...
      begin
        ...
        if IsXPThemesEnabled and
          ...
          hhTheme := 0 //OpenThemeData(0, 'Header')
        else
          hhTheme := 0;
        }
        {
...
var
  i                   : Integer;
  mLarguraColunas     : Integer;
  mLarguraGrid        : Integer;
  NCMet               : TNonClientMetrics;
  mLarguraCol         : Integer;
  StartPoint          : Integer;
  mPosicao            : Integer;
  mColunas            : Array of TColunas;
  mQtdeColunas        : Integer;
  mQtde               : Integer;
  mColunaID           : Integer;
  mLargMaxima         : Integer;
...
begin
  ...
  begin
    ...
    try
      ...
      mLarguraGrid := Width - (NCMet.iBorderWidth * 2) - (GridLineWidth* (ColCount + 1));
      ...
      if (GetWindowlong(Self.Handle, GWL_STYLE) and WS_VSCROLL) <> 0 then
        mLarguraGrid := mLarguraGrid - NCMet.iScrollWidth;

      // Subtract the indicator if it is being used
      if dgIndicator in Options then
      begin
        StartPoint := 1;
        mLarguraGrid := mLarguraGrid - WidthOfIndicator;
      end
      else
        StartPoint := 0;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              SetLength(mColunas, length(mColunas) + 1);
              mColunas[high(mColunas)].Limpar;
              mColunas[high(mColunas)].Coluna        := i;
              mColunas[high(mColunas)].Caption       := Columns[i].Title.Caption;
              mColunas[high(mColunas)].AutoAjustar   := Columns[i].ColunaAjuste.ColAutoWidth;
              mColunas[high(mColunas)].ColMinSize    := Columns[i].ColunaAjuste.ColMinSize;
              mColunas[high(mColunas)].ColMaxSize    := Columns[i].ColunaAjuste.ColMaxSize;
              mColunas[high(mColunas)].Width         := Columns[i].Width;
              mColunas[high(mColunas)].LarguraMaxima := 0;
          ...
          begin
            mLarguraGrid := mLarguraGrid - Columns[i].Width; // Desconto as colunas que não estão definidas como "AutoWidth = True";
          end;
        end
      end;
      ...
      // ----------------------------------------------------------------------------------------------
      mQtdeColunas := ifthen(mQtdeColunas < 1, 1, mQtdeColunas);
      mLarguraCol  := (mLarguraGrid div mQtdeColunas);
      ...
      mColunaID   := 0;
      mPosicao    := 0;
      for i := low(mColunas) to high(mColunas)  do
      begin
        mLargMaxima := 0;
        mColunaID   := mColunas[i].Coluna;
        mPosicao    := mColunaID + StartPoint;
        mLargMaxima := mLarguraCol;

        if (mLarguraCol < mColunas[i].ColMinSize) then
        begin
          mLargMaxima := mColunas[i].ColMinSize;
        end;

        ColWidths[mPosicao]       := mLargMaxima;
        Columns[mColunaID].Width  := mLargMaxima;
      end;
    finally
      FAutoFitIsLocked := False;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        { // Quero implementar aqui uma funcionalidade para formatar string quando forem: CNPJ, CPF, CEP, TELEFONE, etc
        else if (Columns[i].Field.DataType in [ftString]) then
        begin
          // -- Exemplo do que quero implementar --
          if Columns[i].Documento = 'CNPJ' then
          begin
            Mascara := TStringField(Columns[i].Field.DataSet.FieldByName(Columns[i].Field.fieldname)).EditMask;
            if (Mascara <> Columns[i].DisplayFormat) then
              TStringField(Columns[i].Field.DataSet.FieldByName(Columns[i].Field.fieldname)).EditMask := Columns[i].DisplayFormat;
          end;
        end;
        }
      end;
